  ...
  <div class="page">
    ...
    <template v-else-if="stats">
      ...
      <div class="stock-card" style="margin-top: 16px;">
        <div class="ai-header">
          <h3>明牌分析</h3>
          <span class="tip-hint">{{ tipOutcome === 'win' ? '隔日沖賺到了？AI 幫你找出模式存成教訓' : '隔日沖踩雷？AI 分析失敗原因存成避雷教訓' }}</span>
        </div>
        <div class="tip-form">
          <el-segmented
            v-model="tipOutcome"
...
async function runTipAnalysis() {
  ...
  try {
    ...
    await store.analyzeTip(tipExitDate.value, tipSymbol.value.trim(), tipNotes.value.trim(), tipOutcome.value)
  } catch (e) {
    console.error(e)
  }
}
