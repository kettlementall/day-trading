...
class IntradayAiAdvisor
{
     ...
     */
    public function rollingAdvice(string $date, CandidateMonitor $monitor, Collection $allSnapshots, ?array $marketRegime = null): array
    {
        ...
        $userMessage  = $this->buildRollingUserMessage($monitor, $candidate, $stock, $allSnapshots, null, $marketRegime);
     ...
     */
    public function emergencyAdvice(string $date, CandidateMonitor $monitor, Collection $allSnapshots, string $reason, ?array $marketRegime = null): array
    {
        $fallback = ['action' => 'hold', 'notes' => 'AI 不可用，維持現狀', 'adjustments' => null];
        ...
        $systemPrompt = $this->buildRollingSystemPrompt($date, $monitor, $candidate, $stock);
        $userMessage  = $this->buildRollingUserMessage($monitor, $candidate, $stock, $allSnapshots, $reason, $marketRegime);
...
SYSTEM;
     ...
     */
    private function buildRollingUserMessage(
        CandidateMonitor $monitor,
        Candidate $candidate,
        $stock,
        Collection $allSnapshots,
        ?string $emergencyReason = null,
        ?array $marketRegime = null
    ): string
    {
        $candles = $this->aggregateToCandles($allSnapshots);
        $latest = $allSnapshots->sortByDesc('snapshot_time')->first();
...
TASK;
        ...
        $marketRegimeSection = $this->formatSelectedUniverseRegimeSection($marketRegime);
...
MSG;
    ...
    private function formatSelectedUniverseRegimeSection(?array $marketRegime): string
    {
        if (!$marketRegime) {
            return <<<TXT
## 今日候選池盤中環境
...
risk_mode: cautious
summary: 本輪未取得候選池環境摘要，請降低信心並回到單檔結構判斷。此區塊不代表全市場大盤。
TXT;
        }

        $regime = $marketRegime['regime'] ?? 'unknown';
        $source = $marketRegime['source'] ?? 'selected_universe';
        $confidence = $marketRegime['confidence'] ?? 0;
        $entryBias = $marketRegime['entry_bias'] ?? 'wait_confirmation';
        $riskMode = $marketRegime['risk_mode'] ?? 'cautious';
        $summary = $marketRegime['summary'] ?? '無摘要';
...
risk_mode: {$riskMode}
summary: {$summary} 此區塊代表 AI 選入候選池，不等於全市場大盤。
TXT;
    }
