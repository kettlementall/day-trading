...
- **通知**: Telegram Bot（多用戶分級推播：交易信號→全員，系統狀態→僅管理員）
- **部署**: Docker Compose
...
09:35  腿 2 盤中動態加入（4+1 軸聯集 → Fugle 報價 → 規則 → Haiku 快評）
13:25  強制平倉
15:00  盤後結果回填
...
# 補上 stocks.industry（產業別，供類股強弱/新聞配對使用；排程每週一 06:00 自動跑，--force 可覆蓋既有值）
docker compose exec php php artisan stock:fill-industry [--force]

# 當沖複合分數 dry-run（不寫入 DB，輸出「5日均量榜 vs 複合分數榜」對比 + 觀察名單排名）
docker compose exec php php artisan stock:dry-run-screener --date=2026-04-29 --watch=1597,2049 --top=20 --max=100

# 腿 2 盤中動態加入（排程 09:35 自動跑，手動觸發可加 --date）
