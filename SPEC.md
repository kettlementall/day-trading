...
Prompt 包含：時段標籤（09:05~13:25）、昨日收盤（建倉參考）、建議買入價、原始目標/停損、當前目標/停損、Fugle 開高低收及量、開盤跳空%（差異標籤五段細分：顯著超預期/小幅超預期/符合預期/小幅偏弱（雜訊範圍）/顯著不及預期）、距離買入%、5 分 K 線（快照或 Fugle fallback）、先前調整紀錄（最多3筆）。`max_tokens=768` 確保 reasoning 能完整呈現「策略檢核+正負向訊號+結論」三段。
