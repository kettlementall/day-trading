...
Prompt 包含：時段標籤（09:05~13:15）、昨日收盤（建倉參考）、建議買入價、原始目標/停損、當前目標/停損、Fugle 開高低收及量、開盤跳空%（差異標籤五段細分：顯著超預期/小幅超預期/符合預期/小幅偏弱（雜訊範圍）/顯著不及預期）、距離買入%、5 分 K 線（快照或 Fugle fallback）、先前調整紀錄（最多3筆）。`max_tokens=384` 確保 reasoning 能完整呈現「策略檢核+正負向訊號+結論」三段。

System prompt 內含「**進場策略容忍度框架**」（依本次持倉的 `entry_type` 套用 `gap_up_open` / `pullback_entry` / `open_follow_through` / `limit_up_chase` 各自的開盤跳空、首根 K 表現、量比門檻；`pullback_entry` 與 `open_follow_through` 跳空 < -2% 即視為策略失效），以及「**早盤觀察期紀律（09:05~09:25）**」（除非首根 5 分 K 直接跌破停損、或現價對昨收跌幅 > 3%，否則優先 hold/adjust）。容忍度框架優先於早盤紀律：若策略已判定失效，早盤紀律不覆蓋。設計動機：避免 AI 在微幅負跳空（例如預期 +1% 實際 -0.5%）時把策略容忍範圍誤判為策略失效訊號而過早出場，但對開盤崩跌仍保持果斷。
...
### 9.5 機械停損觸發時的 AI 完整分析

`SwingPositionUpdateService::buildAdvice()` 過去在 `close <= current_stop` 時直接 short-circuit 回傳極簡 advice（reasoning 只有一句「收盤 X 跌破停損 Y，建議出場。」），不呼叫 AI。導致觸停損的持倉前端卡片資訊量遠少於正常持倉。

現改為：

1. **停損仍由機械判斷**（`$stopHit = $close <= $stop`）並標記為旗標
2. **強制呼叫 AI** 走 askAi 完整流程，傳 `bool $stopHit=true`
3. Prompt 內插一段「機械停損已觸發」覆蓋區塊，明確要求：
   - reasoning ≥ 80 字（含技術破位細節 / 論點失效狀態 / 籌碼面）
   - `volume_price_signal`：今日量價型態描述
   - `chip_risk_notes`：2-3 條具體數字
   - `eta_reasoning`：是否值得回補 + 觸發條件
   - `target_price_reasoning`：原目標失效說明 + 新目標重設邏輯
4. AI 回傳後，**PHP 強制 override**：`action='exit'`、`technical_health='broken'`、`risk_pressure='high'`（安全網，避免 AI 一時誤判改回 hold）
5. AI 呼叫失敗才退回舊極簡訊息，並附註「AI 分析失敗，請人工檢視」提醒
