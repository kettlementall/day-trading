...
**Feature 5：出場偏誤校正（人設雙向化 + 出場舉證責任，2026-06-02）**

Feature 1–4 補了大盤/軌跡/三層歸因等**數據**，但 swing daily review 的 exit 偏誤仍在：近 30 天 37 筆 advice 中 **57% exit、只 19% hold**（§9.5c 開頭另記 14 天 22 筆 exit 中 55% 賣早）。診斷後確認偏誤**不在某句偏空措辭**（prompt 已寫一堆「不可單獨 exit」），而是三個結構性問題：

1. **人設單向**：「穩健派交易顧問」+ 滿屏「停損審查/風險壓力/風險區」→ 整體框架戴風險濾鏡。
2. **舉證不對稱**：喊 exit 喊錯（賣早）沒有代價，喊 hold 然後跌像失職 → AI 避險天性傾向賣。
3. **缺續抱正當性**：規則全在講「何時賣」，幾乎沒有「何時續抱才對」的正面框架。

**修法（純 prompt，A+B，中度力道）**：

- **A. 人設雙向化**（prompt 開頭）：由「穩健派短線交易顧問，盤後給日建議」改為「短線波段持倉顧問，判斷該續抱/調整/減碼/出場；續抱與出場同等重要，**賣早和套牢一樣是錯誤**」。保留穩健精神但移除單向風險守門重心，補上缺失的「賣早」反向代價。
- **B. 出場舉證責任反轉**（`# 進階仲裁` 段首條）：「預設傾向續抱、出場舉證責任」——thesis 仍有效時預設動作是續抱/調整；`action=exit` 前 `reasoning` 必須先回答「為什麼這不是洗盤/可修復回檔」並指出具體破壞證據（thesis 實質失效 / 技術核心破位 / 籌碼結構轉壞其一以上）。複用既有 `why_not_exit` 欄位，不新增 schema。

**邊界（重要，守住出場紀律）**：B 規則同時**雙向夾**——明寫「**這不是要你凹單**：真破壞仍須果斷 exit，不可因想等反彈而續抱」，防矯枉過正成凹單/攤平。本次**不動** action 值域（仍 hold/adjust/trim/exit，不新增 add）、**不動**「current_stop 只能上移」規則、**不動**審查頻率。這些是更大的結構改動，留待看效果後再議。

**已知限制**：純 prompt 改動，效果**無法自動驗證**，需實盤觀察數週才知 exit% / 賣早率有無改善。另：「加碼後停損卡死在加碼前高位」已於 §9.5f 修復（AI 可在 stop 高於成本時 reanchor）；審查頻率（每日問必然製造動作）仍為獨立未解結構問題。
...
### 9.5f 加碼後停損再錨定（解除「停損卡死在加碼前高位」）

**Bug：** `SwingController::addShares()` 加碼時只重算 `entry_price`（加權平均成本）與 `shares`，**不動 `current_stop`**。原停損是依加碼前較高的進場價設的；加碼攤平後均價下移，停損卻原地不動，於是出現 `stop > 平均成本`（如欣興：均價 1013.8、停損 1022.5）。再疊加 `normalizeAdvice` 裡「停損只能上移」的**程式硬鎖**（`$nextStop = max($previousStop, $nextStop)`），AI 即使想把停損拉回成本下方也會被 `max()` 蓋回去 → 停損**永久卡在加碼前高位**。

**修法（決定性自動鬆鎖，不加欄位、不塞公式，符合 §「避免硬閾值」精神）：**

問題本質只有一個：那道「停損只能上移」硬鎖（`$nextStop = max($previousStop, $nextStop)`），在「停損已高於平均成本」這種結構錯誤狀態下**不該繼續鎖**。AI 本來每次複查就會輸出 `current_stop`，所以只要在這個壞狀態自動鬆鎖、讓 AI 給的值生效即可：

- **不在 `addShares` 算停損**：物理層只改均價/股數；「加碼後停損該擺哪」是品質判斷，交給每日 18:50 持倉複查的 Opus。`addShares` 維持原狀。
- **`normalizeAdvice` 自動鬆鎖**：`previousStop > entry_price`（停損高於成本，加碼攤平卡死的結構訊號）時跳過 `max()` 上移鎖，讓 AI 提的 `current_stop` 直接生效；新風險位由 AI 判斷。**不依賴任何新 schema 欄位**——觸發是程式自動偵測的結構不變量，不靠 AI 記得填欄位（決定性，非機率性）。正常持倉（stop 在成本之下）一律照舊防凹單。
- **prompt 配套**：`# 持倉` 區塊在 `stop > 成本` 時注入一條中性事實標籤；`# 基礎約束` 的停損規則補「唯一下移例外」（僅限成本基準改變、不得用於給套牢部位更多下跌空間），引導 AI 在此狀態主動把停損錨回成本下方。

**為何不用「AI 填欄位授權」做法**：曾考慮新增 `stop_reanchor_reason` 欄位、由 AI 填理由才解鎖。但那是**機率性**修復——要 AI 每次都記得填欄位才生效，漏填一次 bug 就重現；且在 code 設「需 AI 授權」閘門，與「結構錯誤就該自動修正」相違。故改為決定性自動鬆鎖。

**邊界**：移動停利（價格曾漲過該價、停損 > 成本屬合法鎖利）此時 AI 走的是正常「上移」路徑，自動鬆鎖只是不再強制上移、不影響 AI 續抬停損。AI 失敗時的 fallback 停損路徑（非 AI）不在本次範圍（罕觸發、無 AI 可判）。既有卡死的欣興部位於下次複查由新邏輯自動再錨定。
